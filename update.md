...
- Representative scripts: [series/series.py](series/series.py), [data-frames/data-frame.py](data-frames/data-frame.py), [selecting/select.py](selecting/select.py), [selecting/prog.py](selecting/prog.py), [filtering/filter.py](filtering/filter.py), [import/import.py](import/import.py), [aggregation/aggregation.py](aggregation/aggregation.py), [cleaning/cleaning.py](cleaning/cleaning.py)

## Day 9 Data cleaning
- Removed unnecessary columns with `df.drop(columns=[...])`
- Handled missing data with `df.dropna(subset=[...])` and `df.fillna({...})`
- Fixed inconsistent values with `replace()`
- Standardized text with `.str.lower()`
- Changed data types with `astype()`
